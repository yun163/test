...
object PerformanceSpec {
  ...
  val config =
    """
      akka.persistence.performance.cycles.warmup = 1000
      akka.persistence.performance.cycles.load = 100000
    """
...
//        cp.confirm()
  ...
  class EventsourcedTestProcessor(name: String) extends PerformanceTestProcessor(name) with EventsourcedProcessor {
    ...
    val receiveCommand: Receive = controlBehavior orElse {
      case cmd:String ⇒ persist(cmd) { cc ⇒
        if (lastSequenceNr % 1000 == 0) print(".")
        if (lastSequenceNr == failAt) throw new TestException("boom")
      }
      case _ => {
        print("Found other things")
      }
    }
...
class PerformanceSpec extends AkkaSpec(PersistenceSpec.config("leveldb", "PerformanceSpec", serialization = "off").withFallback(ConfigFactory.parseString(PerformanceSpec.config))) with PersistenceSpec with ImplicitSender {
  ...
  def stressCommandsourcedProcessor(failAt: Option[Long]): Unit = {
    ...
    val stressType = failAt match {
      case Some(_) => "with Fail"
      case None => "normal"
    }
    expectMsgPF(100 seconds) {
      case throughput: Double ⇒ println(f"\n $stressType%s throughput = $throughput%.2f persistent commands per second")
    }
  }
  ...
  def stressEventsourcedProcessor(failAt: Option[Long]): Unit = {
    ...
    processor ! StopMeasure
    expectMsgPF(100 seconds) {
      case throughput: Double ⇒ println(f"\n $stressType%s throughput = $throughput%.2f persistent events per second")
    }
  }
  ...
  def stressStashingEventsourcedProcessor(): Unit = {
    ...
    processor ! StopMeasure
    expectMsgPF(100 seconds) {
      case throughput: Double ⇒ println(f"\nthroughput = $throughput%.2f persistent Stashing events per second")
    }
  }

    def stressPersistentChannel(): Unit = {
      val channel = system.actorOf(PersistentChannel.props("channel-with-reply", PersistentChannelSettings(replyPersistent = false)))
      val destination = system.actorOf(Props[PerformanceTestDestination])
      1 to warmupCycles foreach { i ⇒ channel ! Deliver(Persistent(s"msg${i}"), destination.path) }
      channel ! Deliver(Persistent(StartMeasure), destination.path)
      1 to loadCycles foreach { i ⇒ channel ! Deliver(Persistent(s"msg${i}"), destination.path) }
...
//      val destination = namedProcessor[CommandsourcedTestProcessor]
//      1 to warmupCycles foreach { i ⇒ channel ! Deliver(Persistent(s"msg${i}"), destination.path) }
//      channel ! Deliver(Persistent(StartMeasure), destination.path)
//      1 to loadCycles foreach { i ⇒ channel ! Deliver(Persistent(s"msg${i}"), destination.path) }
//      channel ! Deliver(Persistent(StopMeasure), destination.path)
//      expectMsgPF(100 seconds) {
//        case throughput: Double ⇒ println(f"\nthroughput = $throughput%.2f persistent channel messages per second")
//      }
//    }
