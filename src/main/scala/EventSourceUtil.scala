...
object EventSourceUtil extends App {
  ...
  def rankPhoneVerify() {
    implicit val configFile: String = "verify_rank.conf"
    val verifyDumper = new DoDumpEventSource(exportVerifyRank)
  ...
  def exportVerifyRank(messages: List[(Long, Any)], lastSeqNum: Long, processorId: String, exportVerifyRankHdfsDir: String, fs: FileSystem, isEnd: Boolean) {
    val builder = new StringBuilder()
    for ((seqNum, payload) <- messages) {
      if (payload.isInstanceOf[DoUpdateUserProfile]) {
        val profile = payload.asInstanceOf[DoUpdateUserProfile].userProfile
        if (profile.status == UserStatus.Normal && profile.mobileVerified && !writeVerifyRank.verifiedUser.contains(profile.id)) {
//          if (profile.status == UserStatus.Normal && profile.mobileVerified) {
            builder ++= s"${String.valueOf(writeVerifyRank.lastRank)},${String.valueOf(profile.id)}\n"
          writeVerifyRank.lastRank += 1
          writeVerifyRank.verifiedUser.add(profile.id)
        }
      }
    }
    writeVerifyRank.writeRank(builder.toString(), exportVerifyRankHdfsDir, fs, isEnd)
  }
}
